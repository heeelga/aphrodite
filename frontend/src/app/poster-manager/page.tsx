...
export default function PosterManagerPage() {
  ...
  const handleModalPosterUpdate = (itemId: string, newPosterUrl: string) => {
    ...
    const isBadged = newPosterUrl.includes('?badged=')
    
    setItems(prevItems => 
      prevItems.map(item => {
        if (item.id === itemId) {
          // Force a complete re-render by updating the key and URL
          const timestamp = Date.now()
          const cacheBustedUrl = newPosterUrl.includes('?') 
            ? `${newPosterUrl}&t=${timestamp}`
            : `${newPosterUrl}?t=${timestamp}`
          
          return {
            ...item,
            poster_url: cacheBustedUrl,
            badge_status: isRestoration ? 'ORIGINAL' : (isBadged ? 'BADGED' : item.badge_status)
          }
        }
        return item
      })
